 ...
 * @since 1.0.0
 */
class AddressViewBlock extends \app\blocks\FormBlock //\luya\forms\blocks\SummaryBlock
{
    ...
    {
        return 'Preview Delivery';  //Yii::t('forms', 'Summary');
    }
  ...
  /*  public function getSummary()
    ...
    public function getSummary()
    {
        Yii::$app->forms->loadModel();
        $html = null;
        $html .= "<h4>SELECTED ADDRESS:</h4><br>";
        $model = Yii::$app->forms->model;
        foreach ($model->attributes as $k => $v) {
            if ($model->isAttributeInvisible($k)) {
                continue;
